...
for (const sc of scenarios) {
  ...
  await page.evaluate(() => {
    ...
    // The black-frame watchdog samples the first few seconds, which on this
    // rasteriser are the title overlay rather than the scene; give it another
    // look now that the world is actually being drawn.
    window.game.engine.watchdogChecks = 12;
    window.game.engine.watchdogTimer = 0;
  });
  ...
  const shot = await page.evaluate(() => {
    ...
    return {
      ...
      tris: cost.tris,
      calls: cost.calls,
      bloom: game.engine.bloomPass.enabled,
  ...
  console.log(`wrote ${sc.id}.png  via=${shot.used} bloom=${shot.bloom} fps=${shot.fps} tris=${shot.tris} calls=${shot.calls} biome=${shot.biome} v=${shot.speed}`);
  await page.close();
}
